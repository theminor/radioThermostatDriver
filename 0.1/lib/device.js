...
function Device() {
	...
	this.D = 1; // 1 is Block Temperature device
...
Device.prototype.write = function(dataTxt) {
	...
	// OR one the following special strings: "update" will submit an immediate update of the device; and number will set the temperature only
	console.log("[radioThermostatDriver] Data command received from the Ninja Platform: " + dataTxt);
	var self = this;
	if (dataTxt == "update") { updateDevice(self) }
	else if (isNaN(dataTxt)) {  // not a number, so we parse as a JSON string
        ...
        else {  // it is a number, so set the temp
                var setNum = Number(dataTxt);
                console.log("[radioThermostatDriver] Setting Temperature to : " + setNum);
                rslt =  exec("curl -d '{\"t_heat\":" + setNum + ",\"t_cool\":" + setNum + "}' http://" + ipAddressOfThermostat + "/tstat", function (error, stdout, stderr) {
                                stdout.replace(/(\n|\r|\r\n)$/, '');
                                console.log("[radioThermostatDriver] Result: " + stdout);
                                });
                setTimeout( function() { updateDevice(self) }, pauseAfterSetToUpdate);
